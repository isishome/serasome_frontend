  ...
  <div id="app">
    <q-layout view="hHh lpR lfr" v-if="!independent">
      <q-header height-hint="98" class="q-dark q-py-xs" elevated>
        <q-toolbar>
          ...
          <div class="row justify-start items-center nunito">
            ...
            >
              ...
              <span class="col q-ml-sm text-teal-4 gt-sm font-logo"
                >SERA<span class="text-amber-7">SOME</span></span
              >
            </q-btn>
          </div>
          <q-space class="lt-md" />
          <div class="lt-md nunito row justify-center">
            <q-btn flat dense class="row justify-center" @click="home">
              ...
              <span class="q-ml-sm text-h6 text-teal-4 lt-md gt-xs font-logo"
                >SERA<span class="text-amber-7">SOME</span></span
              >
              <span class="q-ml-sm text-h6 text-teal-4 lt-sm font-logo"
                >S'<span class="text-amber-7"
                  >S<span class="text-caption font-logo">ome</span></span
                ></span
              >
            </q-btn>
          </div>
          ...
          <div class="row justify-end items-center q-gutter-x-sm">
            ...
            <q-btn
              dense
              flat
              :disable="processSignOut"
              class="gt-sm"
              @click="sign"
            >
              <q-icon
                :name="signStatus ? 'logout' : 'account_circle'"
                size="20px"
              />
              <span class="q-ml-sm gt-lg">{{
        ...
        <!-- tabs -->
        <q-tabs
          class="nunito text-weight-bold gt-sm"
          narrow-indicator
        ...
        >
          ...
          <q-route-tab v-if="currentSome" :to="`/@${currentSome.name}`">
            <q-icon
              :name="currentSome.icon ? currentSome.icon : 'help'"
              class="q-ma-sm"
              size="20px"
              color="amber-7"
            />
        ...
        <q-scroll-area class="fit">
          <q-list>
            ...
            <q-item clickable @click="goD2R">
              ...
              <q-item-section> D2R </q-item-section>
            </q-item>
            <q-separator />
        ...
        <div :class="['row q-mx-sm', $q.screen.lt.md ? 'q-mt-sm' : 'q-mt-lg']">
          <div class="gt-sm col row justify-end" style="padding: 90px 6px 0 0">
            <adsense
              ...
              data-ad-slot="7331759838"
              :width="$q.screen.gt.md ? '160px' : '120px'"
              height="600px"
              horizontal="right"
              :key="`al-${key}`"
            >
      ...
      <q-footer
        v-if="$q.screen.gt.sm"
        :class="$q.dark.isActive ? 'q-dark' : 'bg-grey-2 text-grey-7'"
        class="ss-footer row justify-between items-center"
        height-hint="56"
      >
        <div class="col-4 gt-sm"></div>
        <div
          class="col q-ma-none col-md-4 row justify-center items-center q-gutter-x-xs text-caption"
        >
          <q-avatar size="md">
            <img src="@/assets/images/seras.svg" />
          </q-avatar>
          ...
          <div class="absolute-right q-mr-lg row items-center">
            <a class="mailto" href="mailto:serasomething@gmail.com">{{
              $t("mailto")
            }}</a>
          </div>
        </div>
        <div class="col gt-sm row justify-end q-ma-sm"></div>
      </q-footer>
    </q-layout>
    <router-view name="independent" v-else />
    <q-dialog v-model="notice.open" persistent>
      <q-card>
        <q-card-section class="bg-secondary text-white text-weight-bold">
          <div class="text-h6">Sera's Something 서비스 종료 안내</div>
        </q-card-section>
        <q-card-section>
          <p>안녕하세요. SeraSome입니다.</p>
          <p>
            Sera's Something 서비스를 2024년 2월 29일 종료하게 되어 안내
            드립니다.
          </p>
          <p>지금까지 Sera's Something 서비스를 이용해 주셔서 감사드리며,</p>
          <p>더 좋은 서비스로 찾아뵐 수 있도록 노력하겠습니다.</p>
          <p>ㅤ</p>
          <p>감사합니다.</p>
        </q-card-section>
        <q-separator />
...
export default {
  ...
  data() {
    return {
      ...
      scrollVisible: false,
      lang: this.$q.cookies.has(process.env.VUE_APP_LANGUAGE_NAME)
        ? this.$q.cookies.get(process.env.VUE_APP_LANGUAGE_NAME)
        : this.$q.lang.getLocale().substring(0, 2) || "ko",
      options: [
        { label: "한국어", value: "ko" },
      ...
      key: 0,
      notice: {
        open: !this.$q.cookies.has("ss.update.20240223"),
        close: false,
      },
    };
  },
  watch: {
    $route: function (to, old) {
      ...
      if (!to.params.sname) this.setCurrentSome(false);
    },
    lang: function (val, old) {
      if (val !== old) {
        this.$q.cookies.set(process.env.VUE_APP_LANGUAGE_NAME, val, {
          path: "/",
          expires: "7300d",
        });
        this.$router.go();
  ...
  computed: {
    ...
    writeCheck() {
      return (
        ...
        this.someList.find((s) => s.name === this.$route.params.sname) &&
        this.someList.find((s) => s.name === this.$route.params.sname).auth
          .write
      );
    },
  },
  created() {
    this.checkStatus();
    const cookieIsDark = this.$q.cookies.has(process.env.VUE_APP_DARK_NAME)
      ? this.$q.cookies.get(process.env.VUE_APP_DARK_NAME)
      : false;
  ...
  methods: {
    ...
    search() {
      if (this.text.trim() !== "" && this.text.trim().length <= 20)
        this.$router
          .push({ name: "search", params: { text: this.text } })
          .catch(() => {});
    },
    forcedMove() {
      if (
        (this.signStatus === true && this.routeName === "sign") ||
        (this.signStatus !== true &&
          (this.routeName === "info" ||
            this.routeName === "post-add" ||
            this.routeName === "post-modify"))
      )
    ...
    sign() {
      ...
      if (this.signStatus === true) {
        ...
        vm.$router
          .push({
            name: "sign",
            params: { redirect: encodeURIComponent(vm.$route.path) },
          })
          .catch(() => {});
    },
    goD2R() {
    ...
    close() {
      this.$q.cookies.set("ss.update.20240223", "confirm", {
        expires: 1,
        path: "/",
      });
