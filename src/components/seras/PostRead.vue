          ...
          <q-card-section class="full-width q-py-sm overflow-hidden gt-sm">
            <q-list class="q-py-none q-px-md">
              ...
              <q-item dense class="no-padding absolute-bottom q-mr-md row justify-end items-center q-gutter-x-xs">
                <q-icon class="q-px-none" size="14px" name="schedule" />
                <div class="text-caption">{{ parsDateTime(postInfo.reg_date) }}</div>
                <div class="text-grey-5">·</div>
                <q-icon class="q-px-none" size="14px" name="visibility" />
                <div class="text-caption">{{ postInfo.seq }}</div>
                <div class="text-grey-5">·</div>
                <q-icon class="q-px-none" size="14px" name="sms" />
            ...
            <q-scroll-area ref="scrollArea" :thumb-style="thumbStyle" class="fit" @scroll="onScroll">
              ...
              <div v-if="postInfo.files && postInfo.files.length > 0">
                ...
                <div ref="attach" class="attach-back row justify-start items-center">
                  <div class="col-4 text-amber-7 text-left q-pl-md"><span class="lt-md">{{ $t('postRead.attachments')
                  }}</span></div>
                  <div class="col text-amber-7 text-center"><span class="gt-sm">{{ $t('postRead.attachments') }}</span>
                  </div>
                  <div class="col-4 text-right q-pr-md">
                    <q-toggle v-model="showAttach" color="amber-7" icon-color="grey-7" icon="attach_file" />
                  </div>
                </div>
                ...
                <div>
                  <q-fab class="all-pointer-events" dense color="brown" push icon="keyboard_arrow_right"
                    ...
                    <q-fab-action glossy push padding="2px" v-if="postInfo.files && postInfo.files.length > 0"
                      color="amber-7" text-color="black" @click="goAttach" icon="attach_file" />
                    ...
                    <q-fab-action glossy push padding="2px" v-if="!isPost" color="purple-3" text-color="black"
                      @click="goSome(postInfo.sname)" icon="arrow_circle_right" />
                  </q-fab>
                </div>
              </div>
              <div>
